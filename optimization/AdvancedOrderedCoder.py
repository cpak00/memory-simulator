...
class AdvancedOrderedCoder(Coder):
    def __init__(self, shift_per_write=4, compare_time=10, energy_rate_l=2/8, energy_rate_h=6/8, endurance_rate=5/8):
        self.shifting = 0
        self.compare_flag = 0
        self.shift_per_write = shift_per_write
        self.compare_time = compare_time
        self.energy_rate_l = energy_rate_l
        self.energy_rate_h = energy_rate_h
        self.endurance_rate = endurance_rate
    ...
    def encode(self, addr, raw):
        ...
        if (self.compare_flag < self.compare_time):
            ...
            if cost_energy <= (self.mm.bits * self.energy_rate_l):
                # self.shifting_content[addr] = self.shifting
                raw = current_row
            elif cost_energy >= (self.mm.bits * self.energy_rate_h):
                ...
                if (cost_endurance < self.mm.bits * self.endurance_rate):
                    raw = current_row
                else:
                    compare_row = int(self.mm.parser.bcs_r(current_row, self.shift_per_write))
                    self.shifting = (self.shifting + self.shift_per_write) % 32
